...
class CartBody extends React.Component {
    ...
    getSubTotal = (data) => {
        ...
        for (let i = 0; i < keys.length; i++) {
            let tempKey = keys[i];
            let tempPrice = realData[tempKey].price_rand;
    ...
    onCheckout = () => {
        ...
        console.log("setCheckoutInfo", sendData);
                                                    ...
                                                    return(
                                                        <tr key={idx} className="pr-list-view">
                                                            <td className="product-thumbnail">
                                                                ...
                                                                <div className="pr-lst-t" style={{float : 'left', paddingLeft : '20px'}}>
                                                                    ...
                                                                    <Rating
                                                                        style={{zIndex:'0'}}
                                                                        name='rate1'
                                                                        defaultValue={parseFloat(this.state.cartData[data].ratingValue)}
                                                                        readOnly={true}
                                                                        precision={0.1}
                                                                        emptyIcon={<StarBorderIcon fontSize="inherit"/>}
                                                                    />
                                                                    {
                                                                        this.state.cartData[data].oldPrice_rand !== 0 &&
                                                                        <h5 style={{textDecoration: 'line-through'}}>R{this.state.cartData[data].oldPrice_rand}</h5>
                                                                    }
                                                                </div>
                                                            </td>

                                                            <td className="product-price">
                                                                <span className="unit-amount">R{this.state.cartData[data].price_rand}</span>
                                                            </td>
                                                            ...
                                                            <td className="product-subtotal">
                                                                <span className="subtotal-amount">R{this.state.cartData[data].price_rand * this.state.cartData[data].quantity}</span>
                                                            </td>
